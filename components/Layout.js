...
export default function Layout({children}) {
  ...
  if (!session) {
    return ( 
    <div className="bg-teal-600 w-screen h-screen flex items-center">
    <div className="text-center w-full">
      <button onClick={() => signIn('google')} className="bg-white p-2 px-4 rounded-lg">Zaloguj się za pomocą Google</button>
      </div>
    </div>
    );
  }
